...
# DiTSR v7 Training Script (Refined for Paper Submission)
# Optimization:
# 1. Latent Space L1 (Avoids VAE boundary artifacts & saves VRAM).
# 2. Robust LPIPS (Center cropped decoding to remove padding noise).
# 3. No Degradation Consistency (Avoids domain gap artifacts).
# 4. Input Noise Augmentation (Robustness).
# 5. Fixed ValueError in DegradationPipeline (Strictly returns 2 values).
...
# [Optimized] Use null embed for pure restoration task
...
# [Optimized] Augment input condition noise to prevent overfitting to VAE distribution
# Defense against Cut 6 (Distribution Mismatch)
LR_LATENT_NOISE_STD = 0.05 

# [Curriculum Logic]
# Step 0-3500: Latent L1 only (Structure)
# Step 3500+: Add LPIPS (Perception)
WARMUP_STEPS = 0
RAMP_UP_STEPS = 3500
TARGET_LPIPS_WEIGHT = 0.50  # Increased slightly since we removed Pixel L1
LPIPS_BASE_WEIGHT = 0.01
# [Optimized] Weight for Latent L1
L1_BASE_WEIGHT = 1.0
...
# [Optimized] Removed Degradation Consistency (Defense against Cut 2)
USE_LR_CONSISTENCY = False 

# Keep Noise Consistency (Optional, keeps z0 stable)
...
def main():
    ...
    for epoch in range(ep_start, 1000):
        ...
        for i, batch in enumerate(pbar):
            ...
            # [Defense against Cut 6] Add noise to conditioning latent to prevent overfitting
            ...
            with torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
                # Always assume FORCE_DROP_TEXT for restoration task
                ...
                # [Defense against Cut 4] Latent L1 Loss
                # Replace VAE Pixel Loss with Latent Loss to avoid boundary artifacts and save memory.
                loss_latent_l1 = F.l1_loss(z0, zh.float())

                # [Defense against Cut 1] Safe LPIPS
                # Decode larger crop -> Crop center -> Compute LPIPS
                # This prevents VAE padding zeros from polluting gradients.
                if w['lpips'] > 0:
                    # Crop 40x40 patch (Latent)
                    top = torch.randint(0, 25, (1,), device=DEVICE).item() # 64 - 40 = 24 max index
                    ...
                    # Center Crop 256x256 (Remove 32px border from each side)
                    # 40*8 = 320. (320-256)/2 = 32.
                ...
                # [Defense against Cut 6 Part 2] Noise Consistency
                # Keeping this as it stabilizes z0 prediction without domain gap issues
