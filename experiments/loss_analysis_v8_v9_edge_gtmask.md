...
### v8（当前 edge_gtmask 线）损失组成（按时间）
- **早期（0~4k）**：`L_v + L_latent_l1` 为主，`LPIPS/edge/flat`基本不生效。  
  **原因**：先把“结构与内容对齐”学稳，避免一开始就被感知项带偏。
- **中期（4k~9k）**：`LPIPS` 线性升到目标值，同时 `edge/flat` 开始 ramp。  
  **原因**：逐步加细节，但只允许在 GT 边缘区域锐化，并对平坦/失焦区域施加高频抑制。
- **后期（9k+）**：保持 `L_v + L_latent_l1 + LPIPS + edge/flat` 的平衡。  
  **原因**：收敛阶段继续稳住结构，同时抑制“无中生有纹理”。

### v9（GAN 线）损失组成（按时间）
- **早期（0~2k）**：与 v8 类似，`GAN=0`，主要靠 `L_v + L_latent_l1 (+ 逐步LPIPS)`。  
  **原因**：先让生成器学会可逆重建，避免判别器过早主导。
- **中期（2k~6k）**：`GAN` 从 0 缓慢 ramp（目前脚本目标 0.08），并叠加 `edge/flat` 区域约束。  
  **原因**：GAN 用来补“真实感纹理”，edge/flat 用来限制 GAN 在失焦区域乱锐化。
- **后期（6k+）**：多损失协同（`v + l1 + lpips + gan + edge/flat`）。  
  **原因**：在感知质量和结构保真之间找平衡，防止“全局发糊”或“局部过纹理化”。

### 为什么这么安排（核心逻辑）
1. **先重建、后感知、再对抗**：这是 SR 里最稳的训练节奏。  
2. **edge/flat 区域约束是关键**：你的任务存在失焦区域，不应被统一“补纹理”。  
3. **GAN 后置且低权重**：GAN 是“修饰器”不是“主目标”，权重大了容易出假纹理。
