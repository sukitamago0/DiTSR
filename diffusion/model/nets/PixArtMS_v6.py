...
class PixArtMSBlockV6(nn.Module):
        ...
        **kwargs,
        ...
        if adapter_cond is not None:
            if isinstance(adapter_cond, (tuple, list)) and len(adapter_cond) == 2:
                ...
                adapter_features = adapter_cond

        flat_features = []
        if adapter_features is not None:
            if torch.is_tensor(adapter_features):
                adapter_features = [adapter_features]
            for feat in adapter_features:
                feat_tokens = feat.flatten(2).transpose(1, 2)
                with torch.cuda.amp.autocast(enabled=False):
                    feat_tokens = self.input_adapter_ln(feat_tokens.float())
                flat_features.append(feat_tokens)

            if len(flat_features) == 1 and len(self.injection_layers) > 1:
                flat_features = flat_features * len(self.injection_layers)
            elif len(flat_features) < len(self.injection_layers):
                pad = [flat_features[-1]] * (len(self.injection_layers) - len(flat_features))
                flat_features = flat_features + pad
        ...
        for i, block in enumerate(self.blocks):
            if flat_features and i in self.injection_layers:
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    sb = self.input_adaln[scale_idx](feat.float())
                    adaln_shift, adaln_scale = sb.chunk(2, dim=-1)

                res = self.input_res_proj[scale_idx](feat.float())
                x = x + alpha * res.to(x.dtype)
...
@MODELS.register_module()
def PixArtMSV6_XL_2(**kwargs):
    return PixArtMSV6(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
